...
;(function(){
    ...
    /***************** 幻灯初始化 ******************/
    var swiper, $banner = $('#bannerWraper');
    $(document).ready(function () {
        ...
        swiper = $('.swiper-container').swiper({
            mode:'vertical',
            useCSS3Transforms: isIpadOrIphone(),
            grabCursor: !0,
            followFinger : false,
            speed : 200,
//            initialSlide : 10,
            ...
            onSlideChangeEnd : function(swiper){
                ...
                $(swiper.slides).removeClass('current-page')
                    .eq(swiper.activeIndex)
                    .addClass('current-page');

                $('.ani').find('.page-master,.page-title,.page-decorate,.page-slave,.page-text').removeClass('active');
                $('.ani').find('.page-master').addClass('active');
                clearTimeout(timeoutid);
                timeoutid = setTimeout(function(){
                	$('.ani').find('.page-title,.page-decorate').addClass('active');
                	timeoutid = setTimeout(function(){
                		$('.ani').find('.page-slave').addClass('active');
                		timeoutid = setTimeout(function(){
                			$('.ani').find('.page-text').addClass('active');
                		},700)
                	},500)
                },500);

                $(document.body).css('background',
                    $(swiper.slides[swiper.activeIndex + 1 ]).css('background-color')
                );
    ...
    //    alert(navigator.userAgent);
        ...
        var playBtn = $("#mp3_btn"),
            $playBtnWraper = $('#music_btn_wraper');

        // 防止开始用户点击暂停后再触发fstSet事件
        var useing = false;
        function playMusic() {
            playBtn.removeClass('music_stop');
            mp3.play();
        }
        playMusic();

        $playBtnWraper.on("touchstart", function () {
            useing = true;
            if ( playBtn.hasClass('music_stop') ) {
                playBtn.removeClass('music_stop');
                mp3.play();
            } else {
                playBtn.addClass('music_stop');
                mp3.pause();
            }
        });

        $('.swiper-container').on('touchstart.fstSet', function() {
            if (mp3 && mp3.paused && !useing ) {
                playBtn.removeClass('music_stop');
                mp3.play();
		...
		*/
        ...
        /********weixin,app,h5差异化处理分享页************/
        //获取当前页面环境类型：app，weixin还是浏览器
        var pageType = (function(){
            if(location.hash.indexOf('from=app') != -1){
                return 'app';
            }
            if(detect.weixin){
        ...
        if(pageType == 'h5'){
            ...
            window._bd_share_config = {
                common : {
                    bdText : '装修需要花多少时间 - 土巴兔装修网',
                    bdDesc : '装修需要花多少时间 - 土巴兔装修网',
                    bdUrl : location.href,
                    bdPic : 'http://m.to8to.com/tuiguang/zxsj/css/zxt-wx.jpg'
                },
                share : [{
...
//            $('<script>').attr('src', 'js/lib/share.js').appendTo('head');
//            loadScript('js/lib/share.js');
			$('.bds_qzone').click(function(){
				var qzongUrl = 'http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey',
					url,
					params;
				...
				params = {
					url: location.href,
					title: window._bd_share_config.common.bdText,
					desc: window._bd_share_config.common.bdDesc,
					summary:window._bd_share_config.common.bdDesc,
					site:"",
					// successurl : location.href
				};
				url = qzongUrl + '?' + genParams(params);
				window.open(url);
				return false;
			});
        }else if( pageType == 'weixin' ){
            try{
                loadScript('http://res.wx.qq.com/open/js/jweixin-1.0.0.js', function(){
                    loadScript('js/lib/wx.js', function(){
                        /*********** 微信分享 ************/
                        try {
                            var shareConfig = {
                                title: '装修需要花多少时间 - 土巴兔装修网',
                                desc: '装修需要花多少时间 - 土巴兔装修网',
                                link: location.href, // 分享链接
                                imgUrl: 'http://m.to8to.com/tuiguang/zxsj/css/zxt-wx.jpg' // 分享图标
                            };

                            to8toWx && to8toWx.init({
                                onReady: function() {
                                    // 分享到朋友圈
                                    to8toWx.exec('onMenuShareTimeline', shareConfig);
            ...
            $('#fllow282,.shareto-wrap,#zxtBanner').hide();
        }
        /********weixin,app,h5差异化处理分享页************/
    });
    //end window.load envent handler


    /********弹出层************/
    var $overlay = $('#overlay');
    $('#shareFriend').bind('tap',function(e){
        $overlay.show();
    });
    $overlay.bind('tap', function(){
        $overlay.hide();
    });
    /********弹出层************/
    ...
    var currentCity = getCookie('to8to_wap_tcode');
    currentCity = currentCity ? currentCity : 'sz';
    var bannerurl = 'http://m.to8to.com/' + currentCity + '/zb/index2.html';

    if(detect.weixin){
        bannerurl += '?to8to_from=wechat_article';
    }
    $('#zxtBanner').attr('href',bannerurl);
    /*********定位当前城市发招标*********/
